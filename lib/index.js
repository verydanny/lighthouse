...
function psiApiFetch(url, waitAmount, verbose) {
    return __awaiter(this, void 0, void 0, function* () {
        const wait = (ms) => new Promise(res => setTimeout(res, ms));
        try {
            const res = yield isomorphic_fetch_1.default(`https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${url}`);
            ...
            if (json &&
                ...
                const result = Object.keys(auditList).reduce((auditResult, key) => (Object.assign({}, auditResult, { [key.split('|')[1]]: fp_1.pipe(fp_1.getOr('Data not found', key.split('|')[0]), fp_1.pick(auditConstants))(base) })), {});
                verbose && console.table(result);
                yield wait(waitAmount);
                return result;
            }
...
function getResults(arr, waitAmount, api, verbose) {
    return __awaiter(this, void 0, void 0, function* () {
        return arr.reduce((previous, curr, index) => __awaiter(this, void 0, void 0, function* () {
            const collection = yield previous;
            ...
            return (Object.assign({}, collection, { [`${curr}-${index}`]: yield psiApiFetch(fakeUrlChange, waitAmount, verbose) }));
        }), {});
    });
}
/**
 ...
 */
function lighthouseRunner({ runs = 1, wait = 2000, view = 'both', api = '', verbose = false, }, ...urls) {
    return __awaiter(this, void 0, void 0, function* () {
        // normalize runs for later
        runs = runs * urls.length;
        verbose && console.log(`Runs: ${runs}\nWaiting: ${wait}\nTesting View: ${view}`);
        const results = yield getResults(createArr({ runs, view, urls }), wait, api, verbose);
        return results;
    });
}
